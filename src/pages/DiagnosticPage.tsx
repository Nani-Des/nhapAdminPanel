...
const RUN_PHASES: { key: DiagnosticRunPhase; label: string }[] = [
...
const ProgressStepper: React.FC<{ current: DiagnosticRunPhase }> = ({ current }) => {
  const currentIdx = RUN_PHASES.findIndex((p) => p.key === current);
  return (
    <div className="w-full" role="progressbar" aria-valuenow={currentIdx + 1} aria-valuemax={RUN_PHASES.length}>
          ...
          return (
            <div key={phase.key} className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1 mt-1.5">
                {done && <CheckCircle2 className="w-3 h-3 text-teal-500 shrink-0" />}
                {active && <Loader2 className="w-3 h-3 text-teal-500 animate-spin shrink-0" />}
                <span
        ...
        <div className="min-h-[60vh] flex items-center justify-center p-6">
          <Card className="max-w-md w-full border-amber-200/80 bg-gradient-to-br from-amber-50 to-white shadow-xl rounded-2xl">
            <CardContent className="py-10 px-8 text-center">
              ...
              <h2 className="text-xl font-bold text-amber-950 mb-2">Hospital not connected</h2>
              <p className="text-sm text-amber-900/80 leading-relaxed">
                Your account needs to be linked to a hospital before you can use this tool.
                Please contact your administrator so they can assign you to the correct facility.
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 md:py-12 space-y-6">
          ...
          <header className="space-y-4">
            <div className="flex items-start gap-4">
              <div className="shrink-0 h-12 w-12 sm:h-14 sm:w-14 rounded-2xl bg-gradient-to-br from-teal-500 to-teal-700 flex items-center justify-center shadow-lg shadow-teal-600/20">
                <Stethoscope className="h-6 w-6 sm:h-7 sm:w-7 text-white" aria-hidden />
              </div>
              <div className="min-w-0">
                ...
                <p className="text-gray-500 text-sm sm:text-[15px] mt-1 leading-relaxed max-w-xl">
                  Describe the case, optionally attach a referral, and receive structured guidance
                  based on your hospital and the wider network.
                </p>
              </div>
            </div>
            ...
            {isDoctorUser(currentAdmin) && (
              <div className="flex items-center gap-2 bg-teal-50 border border-teal-200/60 rounded-xl px-4 py-2.5">
                ...
                <p className="text-xs sm:text-sm text-teal-700">
                  <span className="font-semibold">Clinician mode</span> — This assistant supports your
          ...
          <details className="group rounded-xl border border-gray-200 bg-white shadow-sm">
            ...
            <div className="px-4 pb-4 pt-1 text-sm text-gray-600 space-y-2 border-t border-gray-100">
              <p>
                Be specific in your note (symptoms, timing, vitals if known). If you add a referral,
                only that record is sent — nothing is invented.
              </p>
              <p className="text-xs text-gray-500">
                Voice typing uses your browser's speech recognition and works best in Chrome or Edge.
                Audio is processed on your device / by your browser vendor — not stored by NHAP.
              </p>
            </div>
          </details>
            ...
            <CardContent className="p-0">
              ...
              <div className="p-5 sm:p-6">
                <div className="flex items-center gap-3 mb-4">
                  <StepBadge n={1} />
                  <div>
                    <h2 className="text-base font-semibold text-gray-900">Patient &amp; clinical picture</h2>
                    <p className="text-xs text-gray-500 mt-0.5">
                      Type or dictate a concise summary of the presenting complaint.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {speech.interimText && (
                    <div className="flex items-start gap-2 text-sm text-teal-600 bg-teal-50/60 rounded-lg px-3 py-2" aria-live="polite">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-end gap-3">
                    <div className="flex-1 min-w-[140px]">
                      <label htmlFor="dictation-lang" className="block text-xs font-medium text-gray-600 mb-1.5">
                        Voice language
                      </label>
                      <select
                        id="dictation-lang"
              ...
              <div className="p-5 sm:p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      If the patient has a referral serial number, look it up to enrich the analysis.
                    </p>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    <div className="flex gap-2 sm:pt-6 shrink-0">
                      <Button
                        ...
                        size="md"
                        icon={loadingReferral ? <Loader2 className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}
                      >
                        Find
                      </Button>
                      {referralContext && (
              ...
              <div className="p-5 sm:p-6 bg-gray-50/50">
                <div className="flex items-center gap-3 mb-4">
                  <StepBadge n={3} />
                  <div>
                    <h2 className="text-base font-semibold text-gray-900">Generate guidance</h2>
                    <p className="text-xs text-gray-500 mt-0.5">
                      We'll match your case against this hospital's capabilities and the wider network.
                    </p>
                  </div>
                </div>
                ...
                {running && runPhase !== 'idle' && (
                  <div className="mb-4 p-4 rounded-xl border border-teal-200/60 bg-white">
                    ...
                    <ProgressStepper current={runPhase} />
                  </div>
                )}
          ...
          {(result != null || resultError != null) && (
            <div className="space-y-4 pt-2">
                ...
                <Card className="rounded-2xl border-red-200 shadow-md overflow-hidden">
                  <CardContent className="p-5">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="font-semibold text-red-900">{resultError.title}</p>
                    ...
                    {ch && (
                      <>
                        <div className="rounded-xl bg-teal-50 border border-teal-100 px-3 py-2.5">
                          <p className="text-[11px] font-medium text-teal-600 uppercase tracking-wider">Services</p>
                          <p className="text-lg font-bold text-teal-900 mt-0.5">{ch.services}</p>
                        </div>
                        <div className="rounded-xl bg-teal-50 border border-teal-100 px-3 py-2.5">
                          <p className="text-[11px] font-medium text-teal-600 uppercase tracking-wider">Equipment</p>
                          <p className="text-lg font-bold text-teal-900 mt-0.5">{ch.equipment}</p>
                        </div>
                        <div className="rounded-xl bg-teal-50 border border-teal-100 px-3 py-2.5">
                          <p className="text-[11px] font-medium text-teal-600 uppercase tracking-wider">Clinicians</p>
                          <p className="text-lg font-bold text-teal-900 mt-0.5">{ch.physicians}</p>
                        </div>
                      </>
                    )}
                    <div className="rounded-xl bg-slate-50 border border-slate-100 px-3 py-2.5">
                      <p className="text-[11px] font-medium text-slate-500 uppercase tracking-wider">Peer hospitals</p>
                      <p className="text-lg font-bold text-slate-800 mt-0.5">{String(m.peer_hospitals_count)}</p>
                    </div>
                    <div className="rounded-xl bg-slate-50 border border-slate-100 px-3 py-2.5">
                      <p className="text-[11px] font-medium text-slate-500 uppercase tracking-wider">Referral</p>
                      <p className="text-sm font-bold text-slate-800 mt-1">{m.includes_referral ? 'Included' : 'None'}</p>
                    </div>
                    {Number(m.peer_with_quality_notes) > 0 && (
                      <div className="rounded-xl bg-amber-50 border border-amber-200/60 px-3 py-2.5">
                        <p className="text-[11px] font-medium text-amber-600 uppercase tracking-wider">Incomplete data</p>
                        <p className="text-lg font-bold text-amber-800 mt-0.5">{String(m.peer_with_quality_notes)} site(s)</p>
                      </div>
                    )}
                  </div>
                );
              ...
              {/* Model badge */}
              {result && typeof result === 'object' && result !== null && 'model' in result && (
